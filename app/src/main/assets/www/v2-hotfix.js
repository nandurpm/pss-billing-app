(function(){
  ...
  function injectFixStyles(){
    var old=document.getElementById('v2HotfixStyles');
    if(old) old.remove();
    ...
    style.textContent=`
      .brand{
        ...
        aspect-ratio:480/108!important;
        overflow:hidden!important;
        padding:0!important;
        background:transparent!important;
        border:1px solid #c29a45!important;
        border-radius:14px!important;
        box-shadow:0 12px 28px #0005!important;
      }
      .brand img{
        width:100%!important;
      ...
      .mobile-head{
        ...
        background:linear-gradient(180deg,#260032,#1b0024)!important;
      ...
      .mobile-head img{
        ...
        border:1px solid #c29a45!important;
        border-radius:14px!important;
        box-shadow:0 10px 26px #0006!important;
        background:transparent!important;
      }
      .invoice-logo{
        width:100%!important;
      ...
      .toast-message{
        ...
        min-width:240px;max-width:90vw;background:#28132f;color:#fff;
        padding:13px 16px;border-radius:14px;box-shadow:0 18px 45px #0008;
        font-weight:800;text-align:center;opacity:0;pointer-events:none;transition:.2s
      }
      ...
      .today-summary-card{margin-top:16px!important}
      .today-summary-head{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px}
      .today-summary-head h2{margin:0;font-size:18px}
      .today-summary-head span{font-size:12px;color:#7b687f}
      .today-summary-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
      .today-summary-box{
        background:linear-gradient(135deg,#faf3fc,#fff);
        border:1px solid #eadfed;border-radius:16px;padding:15px
      }
      .today-summary-box span{display:block;font-size:12px;color:#7b687f;font-weight:800}
      .today-summary-box strong{display:block;margin-top:7px;font-size:25px;color:#701e82}
  ...
  function installInvoiceGenerator(){
    try{invoiceNo=nextInvoiceNumber;}catch(e){window.invoiceNo=nextInvoiceNumber;}
  ...
  function applyNivethaName(){
    ...
    var reportBody=document.getElementById('serviceReportBody');
    if(reportBody){
      var table=reportBody.closest('table');
      var lastHeader=table&&table.querySelector('thead th:last-child');
      if(lastHeader&&lastHeader.textContent.trim().toUpperCase()==='NIVEDA') lastHeader.textContent='NIVETHA';
    }
  }

  async function migrateSavedNivethaName(){
    ...
    try{
      ...
      for(var i=0;i<bills.length;i++){
        var bill=bills[i];
        if(String(bill.staff||'').toUpperCase()==='NIVEDA'){
          bill.staff='NIVETHA';
          await putBill(bill);
          changed=true;
        }
  ...
  function ensureTodaySummary(){
    ...
    section.innerHTML=`
      <div class="today-summary-head">
        ...
        <span id="todaySummaryDate"></span>
      </div>
      <div class="today-summary-grid">
        <div class="today-summary-box">
          <span>Total Services</span>
          <strong id="todayServiceTotal">0</strong>
        </div>
        <div class="today-summary-box">
          <span>Total Earnings</span>
          <strong id="todayEarningsTotal">₹0.00</strong>
        </div>
      </div>
    `;
  ...
  async function renderTodaySummary(){
    ...
    try{
      ...
      var bills=(await allBills()).filter(function(bill){
        return bill.billDate===dateValue&&bill.status!=='CANCELLED';
      });
      var services=0;
      var earnings=0;
      bills.forEach(function(bill){
        earnings+=Number(bill.grand)||0;
        (bill.items||[]).forEach(function(item){services+=Number(item.qty)||0;});
      });
  ...
  function wrapHistoryRefresh(){
    ...
    }catch(e){console.warn('History summary wrapper pending',e);}
  }
  ...
  async function saveBillFixed(){
    ...
    if(!staffElement||!staffElement.value){
      toast('Select staff: ABITHA or NIVETHA','error');
    ...
    try{
      ...
      var bill=billData();
      if(!bill.invoiceNo){
        bill.invoiceNo=nextInvoiceNumber();
